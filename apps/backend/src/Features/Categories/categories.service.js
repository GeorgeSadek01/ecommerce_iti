...
// 1. Recursive helper function to update all children
...
const update = async (id, data) => {
    ...
    if (data.parentId !== undefined) {
        ...
        let updatedCategory = await Category.findByIdAndUpdate(id, data, {
            new: true,
            runValidators: true
        });
        // 4. Trigger the recursive helper
        await updateDescendantsAncestors(id, data.ancestors);
        
        // Populate the parentId field so it returns as an object
